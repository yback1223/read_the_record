          ...
          {books.map((b) => {
            ...
            return (
              <Link
                key={b.id}
                href={`/books/${b.id}`}
                className={`group flex flex-col rounded-lg px-3 py-2.5 ${
                  active
                    ? "bg-[color:var(--paper)] shadow-[inset_2px_0_0_0_var(--accent)]"
                    : "hover:bg-[color:var(--paper)]"
                }`}
              >
                <span className="serif truncate text-[15px] leading-snug text-[color:var(--ink)]">
                  {b.title}
                </span>
                <span className="mt-0.5 truncate text-[11px] text-[color:var(--ink-soft)]">
